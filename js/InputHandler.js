var InputHandler = Class.extend({
    ...
    gamepadButton: function(index, buttonName) {
        if (this.currentGamepadState !== null) {
            ...
            if (this._gamepadButtonMap[buttonName.toUpperCase()] !== undefined) {
                return this.currentGamepadState[index].buttons[buttonCode].pressed;
            }
            else {
    ...
    gamepadAxisDigital: function(index, axisName) {
        var state = this.gamepadAxis(index, axisName);
        if (state !== false) {
