(function() {
  ...
  var Block = wrect.Entities.Block;

  window.Builder = Class.extend({
    connectionFaces: [],
    rooms: [],
  ...
  //console.log(collisionDirections);
    ...
    createMeteor: function (options) {
      ...
      var meteor = new Meteor(options.name, blockGraphics, {x: options.x, y: options.y, w: options.width, h: options.height, health: options.health, mass: options.mass});
